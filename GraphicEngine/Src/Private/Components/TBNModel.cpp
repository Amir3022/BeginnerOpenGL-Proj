...
TBNModel::TBNModel(const std::string& path, bool bUsesRGB)
{
...
void TBNModel::LoadModel(const std::string& path)
{
	//Create Assimp importer and try to Load the model into scene object
	Assimp::Importer importer;
	const aiScene* scene = importer.ReadFile(path.c_str(), aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);	//Added flag to calculate tangent vectors for all vertices
...
void TBNModel::ProcessNode(aiNode* node, const aiScene* scene)
{
...
TBNMesh TBNModel::ProcessMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	if (mesh && scene)
	{
		...
		if (mesh->mMaterialIndex >= 0)
		{
			...
			std::vector<Texture> normalMaps = LoadMaterialTextures(material, aiTextureType::aiTextureType_HEIGHT, ETextureType::normal);
			textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
		}

		//return generated Mesh
		return TBNMesh(vertices, indices, textures);
	}
	return TBNMesh();
}

std::vector<Texture> TBNModel::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, ETextureType typeEnum)
{
	...
	if (mat)
	{
		...
		for (int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			if (!bSkip)
			{
				...
				bool bShouldUseRGB = typeEnum == ETextureType::normal ? false : bUsesRGB;
				texture.texIndex = EngineUtilities::LoadImageIntoTexture((textureLocalPath).c_str(), false, bShouldUseRGB);
...
void TBNModel::Draw(std::weak_ptr<Shader> shaderRef)
{
...
void TBNModel::SetTransform(const glm::vec3& in_pos, const glm::vec3& in_rot, const glm::vec3& in_scale)
{
