...
{
public:
	TBNModel(const std::string& path, bool bUsesRGB = false);
	...
	std::vector<TBNMesh>& GetMeshes() { return meshes; }

private:
	void LoadModel(const std::string& path);
	void ProcessNode(aiNode* node, const aiScene* scene);
	TBNMesh ProcessMesh(aiMesh* mesh, const aiScene* scene);
	std::vector<Texture> LoadMaterialTextures(aiMaterial* mat, aiTextureType type, ETextureType typeEnum);

private:
	std::vector<TBNMesh> meshes;
